...
v2.0 改动：
- 始终遍历并处理所有 Anytype.* 平行子目录（不再处理脚本自身目录）
- 每个 Anytype.* 目录独立处理，生成独立的 notion_import.zip
- ZIP 输出到各自 Anytype.* 目录内，使每个导出目录自包含
- 支持一次运行处理多个 Anytype 导出
...
用法：
    python notion_import_preprocessor_v2.0.py
...
def discover_all_work_dirs() -> list:
    """
    自动发现所有 Anytype.* 子目录。
    v2.0: 始终只扫描 Anytype.* 子目录，不再处理脚本自身目录。
...
# 输出 ZIP 文件名（每个 Anytype 目录内固定为 notion_import.zip）
ZIP_OUTPUT_NAME = "notion_import.zip"
...
def process_attachments(content: str, assets_dir: pathlib.Path) -> str:
    ...
    图片链接保持原样，.md 内部页面链接保持原样，其余所有本地文件链接视为附件。
    使用贪婪匹配 (.+) 以支持路径中包含括号的文件名。
    """
    lines = content.split('\n')
    processed_lines = []
    ...
    for line in lines:
        stripped = line.strip()
        # 贪婪匹配到行末最后一个 )，支持路径中包含括号（如 加解密(对外).md）
        link_pattern = r'^\[([^\]]+)\]\((.+)\)$'
        ...
        if match:
            ...
            if ext in IMAGE_EXTENSIONS:
                ...
                # .md 内部页面链接保持原样（已由 update_internal_links 处理）
                processed_lines.append(line)
            else:
                # 所有非图片、非 md 的本地文件链接视为附件
...
def create_notion_zip(processed_files: dict, output_path: pathlib.Path,
                     assets_dir: pathlib.Path, source_dir_name: str) -> None:
    """
    创建 Notion 兼容 ZIP 包。
    processed_files: { "新文件名.md": "文件内容", ... }
    assets_dir: 资源文件的绝对目录路径
    source_dir_name: Anytype 源目录名（用于 README 记录）
    """
    # 收集所有被引用的资源文件名
    all_referenced = set()
...
def generate_readme(processed_files: dict, referenced_assets: set,
                  assets_dir: pathlib.Path, source_dir_name: str) -> str:
    """生成导入说明"""
...
def process_anytype_dir(work_dir: pathlib.Path) -> str:
    """
    处理单个 Anytype.* 目录，生成 notion_import.zip 到该目录内。
    ...
    # Step 4: 打包 ZIP 到 Anytype 目录内（用临时文件确保原子性）
    output_zip = work_dir / ZIP_OUTPUT_NAME
    print(f"\n  [4/4] 打包 ZIP → {output_zip.name}...")
    ...
    print(f"  完成: {dir_name}/{output_zip.name} ({zip_size})")
    return str(output_zip)


def main():
    print("=" * 60)
    print("  Notion 导入预处理工具 v2.0")
    print("=" * 60)
    ...
    if not work_dirs:
        ...
        print("  例如：")
        print("    your_folder/")
        print("      notion_import_preprocessor_v2.0.py")
    ...
    for r in results:
        ...
        rel_path = pathlib.Path(r).relative_to(WORK_DIR)
        print(f"  - {rel_path} ({size})")
    print(f"\n  下一步: 将各目录中的 notion_import.zip 上传到 Notion 导入")
